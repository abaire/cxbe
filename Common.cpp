...
int GenerateFilename(char *szNewPath, const char *szNewExtension, const char *szOldPath, const char *szOldExtension) {
  ...
  // locate last \ or / (if there are any)
  {
    for (int c = 0; szNewPath[c] != 0; c++)
      if (szNewPath[c] == '\\' || szNewPath[c] == '/') szFilename = &szNewPath[c + 1];
  }
  ...
  // locate and remove last . (if there are any)
  {
    ...
    for (int c = 0; szFilename[c] != 0; c++)
      if (szFilename[c] == '.') szWorking = &szFilename[c];

    if (CompareString(szWorking, szOldExtension)) *szWorking = '\0';
...
bool CompareString(const char *szA, const char *szB) {
  while (*szA != '\0' && *szB != '\0')
    if (toupper(*szA++) != toupper(*szB++)) return false;
  return *szA == *szB;
}