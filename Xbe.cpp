    ...
    for (uint32 v = 0; v < m_Header.dwSections; v++) {
      ...
      if (sn != 0) {
        for (int b = 0; b < 8; b++) {
          ...
          if (m_szSectionName[v][b] == '\0') break;
        }
      }
...
Xbe::Xbe(class Exe *x_Exe, const char *x_szTitle, bool x_bRetail) {
  ...
  // pass 2
  {
    ...
    // make room for section headers
    {
      ...
      for (uint32 v = 0; v < m_Header.dwSections; v++) {
        ...
        while (s < 8 && x_Exe->m_SectionHeader[v].m_name[s] != '\0') s++;
  ...
  // pass 3
  {
    m_Header.dwPeBaseAddr =
        m_Header.dwBaseAddr + RoundUp(m_Header.dwSizeofHeaders, 0x1000) - x_Exe->m_SectionHeader[0].m_virtual_addr;
    ...
    // write certificate
    {
      ...
      {
        for (uint32 c = 0; c < 0x10; c++) m_Certificate.dwAlternateTitleId[c] = 0;
      }

      // for now we'll just allow any media you could want
      m_Certificate.dwAllowedMedia = XBEIMAGE_MEDIA_TYPE_HARD_DISK | XBEIMAGE_MEDIA_TYPE_DVD_CD |
                                     XBEIMAGE_MEDIA_TYPE_MEDIA_BOARD | XBEIMAGE_MEDIA_TYPE_NONSECURE_HARD_DISK |
                                     XBEIMAGE_MEDIA_TYPE_NONSECURE_MODE;

      // TODO: allow configuration
      m_Certificate.dwGameRegion = XBEIMAGE_GAME_REGION_MANUFACTURING | XBEIMAGE_GAME_REGION_NA |
                                   XBEIMAGE_GAME_REGION_JAPAN | XBEIMAGE_GAME_REGION_RESTOFWORLD;
      ...
      {
        for (uint32 v = 0; v < 0x10; v++) m_Certificate.bzLanKey[v] = m_Certificate.bzSignatureKey[v] = 0;

        for (uint32 x = 0; x < 0x10; x++)
          for (uint32 y = 0; y < 0x10; y++) m_Certificate.bzTitleAlternateSignatureKey[x][y] = 0;
      }
    ...
    *c = '\0';
    ...
    // write section headers / section names
    {
      ...
      for (uint32 v = 0; v < m_Header.dwSections; v++) {
        ...
        if (characteristics & IMAGE_SCN_MEM_WRITE) m_SectionHeader[v].dwFlags.bWritable = true;
        ...
        if (v < m_Header.dwSections - 1)
          m_SectionHeader[v].dwVirtualSize =
              x_Exe->m_SectionHeader[v + 1].m_virtual_addr - x_Exe->m_SectionHeader[v].m_virtual_addr;
        else
        ...
        // calculate sizeof_raw by locating the last non-zero value in the raw section data
        {
          ...
          if (r > 0) {
            r--;
            while (r > 0) {
              if (x_Exe->m_bzSection[v][r--] != 0) break;
            }
          }
  ...
  // pass 4
  {
    m_Header.dwSizeofImage = m_SectionHeader[m_Header.dwSections - 1].dwVirtualAddr +
                             m_SectionHeader[m_Header.dwSections - 1].dwVirtualSize - m_Header.dwBaseAddr;
        ...
        while (v < relo_size) {
          ...
          while (v < block_stop && v < relo_size) {
            ...
            if (type == IMAGE_REL_BASED_HIGHLOW) {
              ...
              if (bzModRVA != 0) *(uint32 *)bzModRVA += dwBaseDiff;
            } else {
...
Xbe::~Xbe() {
  if (m_bzSection != 0) {
    for (uint32 v = 0; v < m_Header.dwSections; v++) delete[] m_bzSection[v];
...
void Xbe::Export(const char *x_szXbeFilename) {
  ...
  // zero pad
  {
    ...
    // write remaining bytes
    {
      ...
      for (uint32 v = 0; v < remaining; v++) szBuffer[v] = 0;
...
void Xbe::DumpInformation(FILE *x_file) {
  ...
  // print digital signature
  {
    ...
    for (int y = 0; y < 16; y++) {
      fprintf(x_file, "\n                                   ");
      for (int x = 0; x < 16; x++) fprintf(x_file, "%.02X", m_Header.pbDigitalSignature[y * 16 + x]);
    }
    fprintf(x_file, "\n                                   </Hex Dump>\n");
  }
  ...
  // print init flags
  {
    fprintf(x_file, "Init Flags                       : 0x%.08X ", *(uint32 *)&m_Header.dwInitFlags);

    if (m_Header.dwInitFlags.bMountUtilityDrive) fprintf(x_file, "[Mount Utility Drive] ");

    if (m_Header.dwInitFlags.bFormatUtilityDrive) fprintf(x_file, "[Format Utility Drive] ");

    if (m_Header.dwInitFlags.bLimit64MB) fprintf(x_file, "[Limit Devkit Run Time Memory to 64MB] ");

    if (!m_Header.dwInitFlags.bDontSetupHarddisk) fprintf(x_file, "[Setup Harddisk] ");
  ...
  fprintf(x_file, "Debug Pathname Address           : 0x%.08X (\"%s\")\n", m_Header.dwDebugPathnameAddr,
          GetAddr(m_Header.dwDebugPathnameAddr));
  fprintf(x_file, "Debug Filename Address           : 0x%.08X (\"%s\")\n", m_Header.dwDebugFilenameAddr,
          GetAddr(m_Header.dwDebugFilenameAddr));
  fprintf(x_file, "Debug Unicode filename Address   : 0x%.08X (L\"%s\")\n", m_Header.dwDebugUnicodeFilenameAddr,
          AsciiFilename);
  ...
  // print LAN key
  {
    ...
    for (int x = 0; x < 16; x++) fprintf(x_file, "%.02X", m_Certificate.bzLanKey[x]);
    fprintf(x_file, "\n");
  }
  ...
  // print signature key
  {
    ...
    for (int x = 0; x < 16; x++) fprintf(x_file, "%.02X", m_Certificate.bzSignatureKey[x]);
    fprintf(x_file, "\n");
  }
  ...
  // print alternate signature keys
  {
    ...
    for (int y = 0; y < 16; y++) {
      fprintf(x_file, "\n                                   ");
      for (int x = 0; x < 16; x++) fprintf(x_file, "%.02X", m_Certificate.bzTitleAlternateSignatureKey[y][x]);
    }
    fprintf(x_file, "\n                                   </Hex Dump>\n");
  }
  ...
  // print section headers
  {
    for (uint32 v = 0; v < m_Header.dwSections; v++) {
      ...
      // print flags
      {
        fprintf(x_file, "Flags                            : 0x%.08X ", *(uint32 *)&m_SectionHeader[v].dwFlags);

        if (m_SectionHeader[v].dwFlags.bWritable) fprintf(x_file, "(Writable) ");

        if (m_SectionHeader[v].dwFlags.bPreload) fprintf(x_file, "(Preload) ");

        if (m_SectionHeader[v].dwFlags.bExecutable) fprintf(x_file, "(Executable) ");

        if (m_SectionHeader[v].dwFlags.bInsertedFile) fprintf(x_file, "(Inserted File) ");

        if (m_SectionHeader[v].dwFlags.bHeadPageRO) fprintf(x_file, "(Head Page RO) ");

        if (m_SectionHeader[v].dwFlags.bTailPageRO) fprintf(x_file, "(Tail Page RO) ");
      ...
      // print section digest
      {
        ...
        for (int s = 0; s < 20; s++) fprintf(x_file, "%.02X", m_SectionHeader[v].bzSectionDigest[s]);
        fprintf(x_file, "\n");
      }
  ...
  // print library versions
  {
    if (m_LibraryVersion == 0 || m_Header.dwLibraryVersions == 0) {
      ...
      for (uint32 v = 0; v < m_Header.dwLibraryVersions; v++) {
        ...
        for (uint32 c = 0; c < 8; c++) tmp[c] = m_LibraryVersion[v].szName[c];

        tmp[8] = '\0';

        fprintf(x_file, "Library Name                     : %s\n", tmp);
        fprintf(x_file, "Version                          : %d.%d.%d\n", m_LibraryVersion[v].wMajorVersion,
                m_LibraryVersion[v].wMinorVersion, m_LibraryVersion[v].wBuildVersion);
...
void Xbe::ImportLogoBitmap(const uint08 x_Gray[100 * 17]) {
  ...
  // encode logo bitmap
  {
    for (uint32 v = 1; v < 100 * 17; LogoSize++) {
      ...
      while (++v < 100 * 17 - 1 && len < 1024 && color == x_Gray[v] >> 4) len++;
  ...
  // check if there is room to save this, if not then throw an error
  {
    ...
    if (RLE == 0) {
      if (GetError() == 0) SetError("Logo bitmap could not be imported (not enough space in file?)", false);
...
void Xbe::ExportLogoBitmap(uint08 x_Gray[100 * 17]) {
  ...
  for (uint32 v = 0; v < dwLength; v++) {
    ...
    for (uint32 j = 0; j < len; j++) {
      ...
      if (o < 100 * 17) x_Gray[o] = (char)(data << 4);  // could use (int)(data * 15.0 + .5);
    }
  }
}
...
uint08 *Xbe::GetAddr(uint32 x_dwVirtualAddress) {
  ...
  if (dwOffs < sizeof(m_Header)) return &((uint08 *)&m_Header)[dwOffs];

  // offset into image header extra bytes
  if (dwOffs < m_Header.dwSizeofHeaders) return (uint08 *)&m_HeaderEx[dwOffs - sizeof(m_Header)];
...
uint08 *Xbe::GetLogoBitmap(uint32 x_dwSize) {
  ...
  if (dwOffs == 0 || dwLength == 0) return 0;
